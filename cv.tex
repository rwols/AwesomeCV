...
\begin{describesection}
  ...
  \rightsideplain{\emph{C++}}
  \leftside{description}
  \rightsideplain{Camera application for the QI Press Controls company. The software communicates with a custom-made board.}
  ...
  \rightsideplain{\emph{C++}}
  \leftside{description}
  \rightsideplain{Server application for the QI Press Controls company. Main library used here is OpenCV.}
  ...
  \rightsideplain{\emph{C++}}
  \leftside{description}
  \rightsideplain{Server application for the QI Press Controls company.}
...
\rightsideplain{agile, waterfall, scrum}
\\
\leftside{Design Patterns}
\rightsideplain{object-oriented, functional, data-driven, factory, abstract factory, lazy initialization, singleton, RAII, facade, flyweight,  command pattern, iterator, the none object, the visitor, thread pool, locking and joining, consumer-producer, strategy pattern, loose coupling,
curiously recurring template pattern, substitution-failure-is-not-an-error.}
\\
\leftside{Tools Experience}
\rightsideplain{Visual Studio, Xcode, Sublime Text (my favourite right now), clang, clang-cl, gcc, cl, msbuild, ninja, cmake, gdb, lldb, docker, npm}
\\
